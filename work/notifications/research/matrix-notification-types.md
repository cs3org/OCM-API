...
Commits and GitHub permalinks are in `research/examples/metadata.md`
(Nextcloud server, Spreed, ownCloud reva for the notify examples, upstream
cs3org/reva for the route-only contrast, and OCM-API for the spec pin).
...
## 3. ownCloud reva `notify` line (`resourceType` `file`)

The table below comes from the pinned owncloud/reva tree. For how ocis wraps
that service and what the HTTP handler expects, see
[`ocis-reva.md`](ocis-reva.md). Upstream cs3org/reva is a different code path
and is covered in `cernbox-reva.md`.

Structs: [`payload.go`](https://github.com/owncloud/reva/blob/9ef2450fc1c9c711cc4e21034aa921bb2d47f34c/pkg/ocm/client/payload.go).

Runtime send: [`ocmshareprovider.go`](https://github.com/owncloud/reva/blob/9ef2450fc1c9c711cc4e21034aa921bb2d47f34c/internal/grpc/services/ocmshareprovider/ocmshareprovider.go) function `notify`.

| notificationType        | Sent by notify switch | Inner notification object                                             |
| ----------------------- | --------------------- | --------------------------------------------------------------------- |
| SHARE_UNSHARED          | yes                   | grantee only (opaque user id)                                         |
| SHARE_CHANGE_PERMISSION | yes                   | grantee + protocol (built by the sender's `getProtocols(ctx, share)`) |

Other `notificationType` strings: default branch in inspected `notify` returns error (see tests in [`pkg/ocm/client`](https://github.com/owncloud/reva/blob/9ef2450fc1c9c711cc4e21034aa921bb2d47f34c/pkg/ocm/client/notify_payload_test.go)).

HTTP: [`client.go`](https://github.com/owncloud/reva/blob/9ef2450fc1c9c711cc4e21034aa921bb2d47f34c/pkg/ocm/client/client.go) `NotifyRemote` -> `POST` `{endpoint}/notifications`.

Pins: `metadata.md` (reva and OCM-API commits).

Permission-change: ownCloud reva sends `SHARE_CHANGE_PERMISSION` and sets `notification.protocol` via [`protocols.go`](https://github.com/owncloud/reva/blob/9ef2450fc1c9c711cc4e21034aa921bb2d47f34c/internal/http/services/ocmd/protocols.go) `Protocols.MarshalJSON`, same object model as Share Creation [`protocol`](https://github.com/cs3org/OCM-API/blob/6b2225e31fc4d50df16aac24a570e8263591130c/spec.yaml#L555-L572).

The sender currently hardcodes `resourceType: "file"` even though the receive
side uses `resourceType` when expanding some folder permissions from WebDAV
protocol data. That is worth treating as a real interoperability caveat for
folder shares.

OpenAPI [`notificationType` MAY list](https://github.com/cs3org/OCM-API/blob/6b2225e31fc4d50df16aac24a570e8263591130c/spec.yaml#L768-L777) and IETF [Share Updating](https://github.com/cs3org/OCM-API/blob/6b2225e31fc4d50df16aac24a570e8263591130c/IETF-RFC.md#L1293-L1301) use `RESHARE_CHANGE_PERMISSION`. `NewNotification` does not document inner `notification` keys. See `research/examples/discrepancies.md`.
